...
function compute_NDVI(image_collection,roi,start_time,end_time,ndvi_threshold){
  ...
  */
  ...
  // Now highlight the healty vegetation, that is, the one with the NDVI > 0.3.
  var healty_vegetation_mask = NDVI.gt(ndvi_threshold);


  // Mask (remove) the pixels with unhealthy vegetation (NDVI < 0.3), 
  ...
  var unmasked_pixel_count_result = unmasked_pixel_count.get("B8");
  
  
  var extensions_of_healty_vegetation_area_m2 = ee.Number(unmasked_pixel_count_result)
  ...
  // Layer to highlight the healty vegetation, that is, the one with the NDVI > 0.3.
  ...
  var extensions_of_roi_m2 = extensions_of_roi_m2.divide(1000).divide(1000);
  print("Area of interest");
  print(extensions_of_roi_m2);
  print("Green area");
...
var green_area_before_storm = compute_NDVI(S2,Areacolpita,"2018-06-01","2018-07-30",0.3);


print("");

// Apply the compute_NDVI function and get the km^2 oh healty vegetation after the storm
var green_area_after_storm = compute_NDVI(S2,Areacolpita,"2019-06-01","2019-07-30",0.3);


print("");
print("Total of green area damaged ndvi 0.3")
...
// Apply the compute_NDVI function and get the km^2 oh healty vegetation before the storm
var green_area_before_storm = compute_NDVI(S2,Areacolpita,"2018-06-01","2018-07-30",0.5);


print("");

// Apply the compute_NDVI function and get the km^2 oh healty vegetation after the storm
var green_area_after_storm = compute_NDVI(S2,Areacolpita,"2019-06-01","2019-07-30",0.5);


print("");
print("Total of green area damaged ndvi 0.5")
