...
function compute_NDVI(image_collection,roi,start_time,end_time,ndvi_threshold){
  ...
  Parameters:
  - image_collection, a Sentinel2 image collection.
  ...
  */
  ...
  var filtered_collection_aggregated = filtered_collection.median().select("B4","B3","B2","B8").clip(roi);
  ...
  Map.addLayer(NDVI,{min:-1,max:1},title_ndvi,false); 
  ...
  Map.addLayer(filtered_collection_aggregated,{bands:["B8","B4","B3"],min:0,max:0.3},title_false_color_composite,false);
  ...
  Map.addLayer(healty_vegetation_mask,{},title_highlight_ndvi,false);
      
  
  // Show the masking for the unhealthy vegation
  var title_masked_aggregated_image = start_time + " / " +  end_time + " masked_aggregated_image";
  Map.addLayer(masked_aggregated_image,{bands:["B4","B3","B2"],min:0,max:0.3},title_masked_aggregated_image,false);
  /*---------------------------*/
  ...
  print("Area of interest",extensions_of_roi_km2);
  print("Green area",extensions_of_healty_vegetation_area_km2);
...
print("Total of green area damaged",green_area_before_storm.subtract(green_area_after_storm));
